...
pub fn share_from_bytes(share: &[u8]) -> Result<Zeroizing<Share>> {
    use vsss_rs::Share as vShare;

    if share.len() != KEY_LEN + 1 {
        return Err(anyhow!("invalid slice length, must be {}", KEY_LEN + 1));
    }
...
// TODO: I want to implement a newtype for Share but I haven't figured out
// the ReadableShareSet trait.
pub fn share_from_hex(share: &str) -> Result<Zeroizing<Share>> {
    use vsss_rs::Share as vShare;

    let share: String = share.chars().filter(|c| !c.is_whitespace()).collect();
    let share = hex::decode(share).context("Decoding bytes from hex")?;
...
mod tests {
    ...
    fn share_from_hex(share: &str) -> Result<Share> {
        ...
        id[id.len() - 1] = share[0];
        let id = id;
        let id = Scalar::from_repr(id.try_into()?)
            .into_option()
            .ok_or(anyhow!("Failed create scalar from identifier byte repr"))?;
        let id = IdentifierPrimeField::from(id);

        // extract the share value and build up the type to hold it
        let value: [u8; KEY_LEN] = share[1..].try_into()?;
        let value = Scalar::from_repr(value.try_into()?)
            .into_option()
            .ok_or(anyhow!("Failed create scalar from value byte repr"))?;
        let value = IdentifierPrimeField::from(value);
        ...
        // combine the identifier and the value into the Share
        use vsss_rs::Share as vShare;
        Ok(Share::with_identifier_and_value(id, value))
    }
